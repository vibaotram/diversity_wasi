...
nj1 <- ggtree(nj_tree, ladderize = F, layout = "ape", color = "gray") %<+% acc_info +
  ...
  #             linesize = .2, size = 5) +
  geom_tippoint(aes(color = group, shape = group, size = group, alpha = group)) +
  guides(color = guide_legend(title = "group")) +
  scale_shape_manual(values = 0:8) +
  scale_color_manual(values = group9_col) +
  scale_size_manual(values = c(rep(5, 8), 2)) +
  scale_alpha_manual(values = c(rep(1, 8), 0.8)) +
  coord_flip() + scale_y_reverse() + scale_x_reverse()

ggtree(hclust(all_dist), ladderize = F, layout = "dendrogram") %<+% acc_info +
  # geom_tiplab(aes(subset = grepl("S-", label), color = group), hjust = -0.5,
  #             linesize = .2, size = 5) +
...
pca2 <- core6 %>% 
  ...
  xlab(paste0("PC1 (", format(eigen[1]/sum(eigen)*100, digits = 2), "%)")) +
  ylab(paste0("PC2 (", format(eigen[2]/sum(eigen)*100, digits = 2), "%)")) +
  scale_color_manual(values = c(as.character(group9_col), "gold3")) + 
  scale_shape_manual(values = c(0:8, 8)) +
  theme_minimal() + 
  theme(panel.border = element_rect(color = "gray", fill = NA),
        axis.title = element_text(size = 12))
```
...
cg_vn <- all_snmf_prop6 %>% 
  filter(group %in% c("O", "B", "R", "E") | (group == "Vietnam" & new_ancestry == 8 & proportion > 0.8)) 
...
ggplot(data = africa) +
  ...
  scale_size_manual(values = c(25, 25, 25, 25, 50, 20, 20))
```
